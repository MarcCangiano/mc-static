...
const CONTACT_SECTION = `<div class="mc-block-group is-nowrap is-layout-flex mc-container-core-group-is-layout-8e2d48a5 mc-block-group-is-layout-flex" style="margin-bottom:26px"><p class="mc-num mc-block-paragraph">07</p><h2 class="mc-block-heading" id="contact">Contact</h2></div><p class="mc-block-paragraph" style="font-size:clamp(17px, 1.5vw, 20px);line-height:1.55">I&#8217;d love to hear from you.</p><form class="mc-form" method="post" action="https://formsubmit.co/me@marccangiano.com"><div class="mc-hp" aria-hidden="true"><label for="mc-hp">Leave this empty</label><input type="text" id="mc-hp" name="_honey" tabindex="-1" autocomplete="new-password" aria-hidden="true" data-lpignore="true" data-1p-ignore="true"></div><div class="mc-row"><div class="mc-f"><label for="mc-name">Name <span class="mc-req">*</span></label><input id="mc-name" type="text" name="name" autocomplete="name" required></div><div class="mc-f"><label for="mc-email">Email <span class="mc-req">*</span></label><input id="mc-email" type="email" name="email" autocomplete="email" spellcheck="false" required></div></div><div class="mc-f"><label for="mc-phone">Phone</label><input id="mc-phone" type="tel" name="phone" autocomplete="tel"></div><div class="mc-f"><label for="mc-message">Message</label><textarea id="mc-message" name="message" rows="6"></textarea></div><div class="mc-check" data-mc-check><input type="checkbox" id="mc-human" name="human" value="confirmed" required><label for="mc-human"><span class="mc-check__box" aria-hidden="true"><svg viewBox="0 0 24 24" class="mc-check__tick"><path d="M5 12.5l4.6 4.6L19 7.2"/></svg></span><span class="mc-check__text">I&#8217;m a person</span></label><span class="mc-check__mark" aria-hidden="true"><svg viewBox="0 0 40 40"><path class="mc-check__ring" d="M20 3.6 L33.5 10.4 V22 C33.5 29.4 27.6 34.6 20 36.9 C12.4 34.6 6.5 29.4 6.5 22 V10.4 Z"/><path class="mc-check__glyph" d="M14.4 24.6 V15.4 M14.4 15.4 L20 22.4 L25.6 15.4 M25.6 15.4 V24.6"/></svg><span>Human check</span></span></div><input type="hidden" name="_elapsed" value=""><input type="hidden" name="_subject" value="marccangiano.com inquiry"><input type="hidden" name="_next" value="https://marccangiano.com/#sent"><input type="hidden" name="_captcha" value="false"><button type="submit">Send</button></form>`;

/* The human check.
 *
 * A checkbox alone stops nothing automated — a script can tick it. What it does
 * is make an accidental or drive-by submission take a deliberate act, and it
 * pairs with the honeypot and the elapsed-time field, which is the part that
 * actually separates people from scripts: a form filled in under two seconds
 * was not filled in by a person.
 *
 * Once ticked it stays ticked. There is no reason to let someone un-say it, and
 * a control that can be switched back off is one more way to press Send on a
 * form that then fails validation.
 */
const CHECK_JS = `
(function () {
...
const FORM_CSS = `.mc-sent{display:none;position:fixed;top:22px;left:50%;transform:translateX(-50%);z-index:9999;box-sizing:border-box;width:min(600px, calc(100% - (var(--mc-gutter) * 2)));padding:14px 20px;background:var(--mc--preset--color--base);border:1px solid var(--mc--preset--color--accent);border-radius:2px;font-family:var(--mc--preset--font-family--mono);font-size:12px;letter-spacing:.06em;line-height:1.6;text-align:center;color:var(--mc--preset--color--contrast);box-shadow:0 8px 30px rgba(0,0,0,.45)}.mc-sent:target{display:block;animation:mc-sent-out 3.6s ease-in forwards}@keyframes mc-sent-out{0%,89%{opacity:1}100%{opacity:0;visibility:hidden}}@media(prefers-reduced-motion:reduce){.mc-sent:target{animation:mc-sent-cut 3s steps(1,end) forwards}@keyframes mc-sent-cut{0%,99%{opacity:1}100%{opacity:0;visibility:hidden}}}}.mc-form{display:grid;gap:22px;margin-top:26px;font-size:clamp(17px, 1.5vw, 20px);max-width:68ch}.mc-form .mc-f{display:grid;gap:9px}.mc-form .mc-row{display:grid;gap:22px;grid-template-columns:1fr 1fr}@media(max-width:640px){.mc-form .mc-row{grid-template-columns:1fr}}.mc-form label{font-family:var(--mc--preset--font-family--mono);font-size:11px;letter-spacing:.1em;text-transform:uppercase;color:var(--mc--preset--color--muted)}.mc-form .mc-req{color:var(--mc--preset--color--accent)}.mc-form input,.mc-form textarea{width:100%;box-sizing:border-box;background:transparent;color:var(--mc--preset--color--contrast);border:1px solid var(--mc--preset--color--line);border-radius:2px;padding:13px 14px;font-family:inherit;font-size:16px;line-height:1.5;transition:border-color .2s ease}.mc-form textarea{min-height:150px;resize:vertical}.mc-form input:focus,.mc-form textarea:focus{outline:none;border-color:var(--mc--preset--color--accent)}.mc-form input:-webkit-autofill{-webkit-text-fill-color:var(--mc--preset--color--contrast);-webkit-box-shadow:0 0 0 1000px var(--mc--preset--color--base) inset}.mc-form button{justify-self:start;background:var(--mc--preset--color--accent);color:var(--mc--preset--color--base);border:1px solid var(--mc--preset--color--accent);border-radius:2px;padding:14px 30px;font-family:var(--mc--preset--font-family--mono);font-size:12px;letter-spacing:.1em;text-transform:uppercase;cursor:pointer;transition:opacity .2s ease}.mc-form button:hover,.mc-form button:focus-visible{opacity:.85}.mc-hp{position:absolute!important;left:-9999px!important;width:1px!important;height:1px!important;overflow:hidden!important}/* fit-content, not a pixel cap: the box is a block-level flex item in the form grid, so it stretched to the column and a max-width was simply ignored when the column was narrower. Sizing to its own content is what actually keeps it short, at any column width. *//* The human check. It borrows the SHAPE people already recognise, a bordered box with a checkbox and a mark off to the right, but the mark and the wording are this site's own. Copying Google's badge or the reCAPTCHA name would be passing off their product as ours. */.mc-check{display:flex;align-items:center;justify-content:space-between;gap:12px;width:fit-content;max-width:100%;padding:11px 13px;border:1px solid var(--mc--preset--color--line);border-radius:3px;background:rgba(255,255,255,.02)}.mc-check input{position:absolute;opacity:0;width:1px;height:1px;margin:0}.mc-check label{display:flex;align-items:center;gap:10px;cursor:pointer;font-family:inherit;font-size:13.5px;letter-spacing:0;text-transform:none;color:var(--mc--preset--color--contrast)}.mc-check__box{position:relative;flex:0 0 22px;width:22px;height:22px;border-radius:2px;border:1px solid var(--mc--preset--color--line);background:transparent;transition:border-color .2s ease,background .2s ease}/* The tick spins in as it draws: a half turn while the stroke lays itself down, settling upright. */.mc-check__tick{position:absolute;inset:0;width:100%;height:100%;fill:none;stroke:var(--mc--preset--color--accent);stroke-width:2.4;stroke-linecap:round;stroke-linejoin:round;stroke-dasharray:24;stroke-dashoffset:24;transform:rotate(-180deg) scale(.55);transform-origin:50% 50%;transition:stroke-dashoffset .42s cubic-bezier(.65,0,.35,1),transform .42s cubic-bezier(.34,1.4,.5,1)}.mc-check input:checked + label .mc-check__box{border-color:var(--mc--preset--color--accent)}.mc-check input:checked + label .mc-check__tick{stroke-dashoffset:0;transform:rotate(0) scale(1)}.mc-check input:focus-visible + label .mc-check__box{outline:2px solid var(--mc--preset--color--accent);outline-offset:2px}/* The badge sits beside the label, not under it. Stacked, it wrapped onto a second line at the width the form actually renders at. */.mc-check__mark{display:flex;flex-direction:row;align-items:center;gap:5px;flex:0 0 auto;font-family:var(--mc--preset--font-family--mono);font-size:8px;letter-spacing:.06em;line-height:1.2;text-transform:uppercase;color:var(--mc--preset--color--muted);pointer-events:none;white-space:nowrap}.mc-check__mark svg{width:20px;height:20px;flex:0 0 20px;fill:none;stroke:var(--mc--preset--color--muted);stroke-width:1.6;stroke-linejoin:round;transition:stroke .25s ease}.mc-check__glyph{stroke-linecap:round}.mc-check.is-done .mc-check__mark svg{stroke:var(--mc--preset--color--accent)}.mc-check.is-done label{cursor:default}.mc-check.is-done .mc-check__box{background:rgba(95,143,0,.12)}@media(prefers-reduced-motion:reduce){.mc-check__tick{transition:none;transform:rotate(0) scale(1)}}/* The four project shots run two to a row, and fall to one full-width column on a phone. They were a single stacked column at every width, which made the section far longer than it needed to be. Appended here rather than editing the theme's own .mc-shot-stack rule, so the two files stay in step. *//* Exactly two to a row. auto-fit was the wrong tool once the stack got the full width: it fits as many as it can and gave four. minmax(0,1fr) rather than 1fr so a long caption cannot push a column past its share. *//* The tool screenshot and the four project shots were columns beside each other, which put the thumbnails to the RIGHT of what-the-server-sent rather than under it, and squeezed them to 183px. One column here, so the shots sit below the screenshot and get the full width to lay out two to a row in. */.mc-shots{grid-template-columns:1fr;gap:28px}.mc-shot-stack{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:22px;align-items:start;align-content:start}/* align-content, not align-items: the stack has a definite height from the layout around it, so the ROWS were stretching to share it and left a 250px hole between them. align-items only governs an item inside its row. */.mc-shot-stack figure{margin:0;min-width:0}/* One shape for all four. The source screenshots are 1.60, 1.77, 1.77 and 2.03 to one, so at equal column widths they rendered 364, 328, 328 and 286 tall — the right-hand pair visibly shorter than the left. A common 16:9 with object-fit:cover crops rather than letterboxes, so none of them sit in a band of empty space. */.mc-shot-stack img{width:100%;aspect-ratio:16/9;height:auto;object-fit:cover;object-position:center;display:block}@media(max-width:620px){.mc-shot-stack{grid-template-columns:1fr;gap:16px}}/* Three of the four carry mc-desk-only, which hides them below 1024px, so a phone only ever saw the first. Scoped to this stack: the class is used elsewhere and should keep meaning what it means there. */.mc-shot-stack .mc-desk-only{display:block}`;
...
const SENT_NOTICE = `<p id="sent" class="mc-sent" role="status">Thanks for reaching out to me.<br>I will respond at the earliest time possible.</p>`;

function injectSentNotice( html ) {
	if ( html.includes( 'id="sent"' ) ) return html;
	const m = html.match( /<body[^>]*>/ );
	if ( ! m ) throw new Error( "no <body> tag; sent notice not injected" );
	const i = m.index + m[ 0 ].length;
	return html.slice( 0, i ) + SENT_NOTICE + html.slice( i );
}
...
async function main() {
  ...
  await fs.writeFile(path.join(OUT, "assets/ui.js"), ui + FORM_JS + CHECK_JS);
  const before = html;
